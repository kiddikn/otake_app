...
urlpatterns = [
    ...
    url(r'^sp/schedule_create/(?P<year>[0-9]+)/(?P<u_id>[0-9]+)$', views.SpShiftCreateView.as_view(), name="sp_shift_view"),
    url(r'^shift_member/(?P<year>[0-9]+)/(?P<month>[0-9]+)/(?P<day>[0-9]+)$',views.ShiftMemberView.as_view(),name='shift_member'),
    url(r'^all_shift_member/(?P<year>[0-9]+)$',views.AllShiftMemberView.as_view(),name='all_shift_member'),
